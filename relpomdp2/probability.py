...
class TabularDistribution(pomdp_py.Histogram):
...
def test_distribution():
    ...
    assert px.prob({'X':'x1'}) == px.prob({'X':'x2'}) == 1.0 / 3
    ...
    assert pxy.prob(('x1','y2')) == pxy.prob(('x3', 'y1'))

    pxgy = pxy.condition({'Y': 'y1'})
    assert pxgy.variables == ["X"]
    assert sum(pxgy.prob(event) for event in pxgy.probs) == 1.0
    print(pxgy)
    ...
    px = pxy.sum_out(["Y"])
    assert px.prob(("x1",)) == (pxy.prob(("x1", "y1")) + pxy.prob(("x1", "y2")))
    assert px.prob(("x2",)) == (pxy.prob(("x2", "y1")) + pxy.prob(("x2", "y2")))
    print(px.to_df())
