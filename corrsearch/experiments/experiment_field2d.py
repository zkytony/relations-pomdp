...
Three experiments:
1. Fix number of objects, randomize detector noise, vary the size of search environment
...
def corr_heuristic_pouct_trial(spec, joint_dist_path, seed, name_prefix, k=-1):
    ...
    pruning = "no-prune" if k <= 0 else "k={}".format(k)
    baseline = "heuristic(%s)" % pruning
