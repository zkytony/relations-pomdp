from corrsearch.objects import ObjectState, OOState
import unittest

class TestObjectStateOOState(unittest.TestCase):
    def setUp(self):
        ...
        self.so3 = ObjectState(3, "bag",
                               {"location": (5,10),
                                "contains": (1, 2),
                                "owner": ("Bob", 40, "worker")})
        self.s = OOState({self.so1, self.so2, self.so3})
    ...
    def test_objid_checking(self):
        ...
        with self.assertRaises(AssertionError):
            OOState({self.so1, self.so2, self.so3, so4})
        so4 = ObjectState(4, "cup", {})
        with self.assertRaises(AssertionError):
            OOState({self.so1.id:self.so1,
                     self.so1.id: so4})
